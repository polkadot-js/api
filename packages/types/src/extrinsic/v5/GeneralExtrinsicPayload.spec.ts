...
import { GeneralExtrinsicPayload as ExtrinsicPayload } from './index.js';
...
describe('GeneralExtrinsicPayload', (): void => {
  it('has a sane inspect', (): void => {
    ...
    expect(new ExtrinsicPayload(registry, { method: tx['timestamp']['set'](0).toHex() } as never).inspect()).toEqual({
      inner: [
        ...
        { name: 'transactionExtensionVersion', outer: [new Uint8Array([0])] }
      ]
    });
  });
});
