...
describe('Blueprint', (): void => {
  ...
  it('can construct with an .contract ABI (v0)', (): void => {
    expect(
      () => new Blueprint(mockApi, v0contractFlipper as Record<string, unknown>, { Existing: '0x1234' }, toPromiseMethod)
    ).not.toThrow();
  });

  it.only('can construct with an .contract ABI (v1)', (): void => {
    expect(
      () => new Blueprint(mockApi, v1contractFlipper as Record<string, unknown>, { Existing: '0x1234' }, toPromiseMethod)
    ).not.toThrow();
  });
});
