...
describe('unsubscribe', () => {
  ...
  beforeEach(() => {
    ...
    return subscribe(state, 'chain_subscribeNewHead', [() => void 0]).then((_id) => {
      id = _id;
    });
  });

  it('fails on unknown ids', () => {
    return unsubscribe(state, 'chain_subscribeNewHead', 5).catch((error) => {
      expect(error.message).toMatch(/Unable to find/);
    });
  });

  it('unsubscribes successfully', () => {
    return unsubscribe(state, 'chain_subscribeNewHead', id);
  });
  ...
  it('clears the subscriptions', () => {
    return unsubscribe(state, 'chain_subscribeNewHead', id).then(() => {
