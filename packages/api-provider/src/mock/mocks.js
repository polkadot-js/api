...
const storage = require('@polkadot/storage-substrate/keys');
...
module.exports = function mocks ({ emitter, storage, subscriptions }: MockState): void {
  ...
  setInterval(() => {
    ...
    keyring.getPairs().forEach(({ publicKey }: KeyringPair, index: number) => {
      setStorageBn(storage, storage.staking.keys.balanceOf, newHead.number.muln(3).iaddn(index), publicKey());
      setStorageBn(storage, storage.system.keys.nonceOf, newHead.number.addn(index), publicKey());
    });

    setStorageBn(storage, storage.timestamp.keys.current, Date.now());
