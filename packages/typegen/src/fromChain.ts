...
import { generateDefaultConsts, generateDefaultEvents, generateDefaultQuery, generateDefaultRpc, generateDefaultTx } from './generate';
  ...
  writeFile(path.join(process.cwd(), output, 'augment-api.ts'), (): string =>
    [
      ...
      ...[
        ...
        ...['consts', 'events','query', 'tx', 'rpc'].filter((key) => !!key).map((key) => `./augment-api-${key}`)
