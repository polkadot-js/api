...
describe.skip('extractStorageArgs', (): void => {
  ...
  it('validates no-arg plain', (): void => {
    expect(
      extractStorageArgs(false, storage.timestamp.now, [])
  ...
  it('validates no-arg plain (failing when there are args)', (): void => {
    expect(
      (): any[] =>
        extractStorageArgs(false, storage.timestamp.now, [123, 456])
    ).toThrow('timestamp.now() does not take any arguments, 2 found');
  });

  it('validates map, 1 arg', (): void => {
    expect(
      extractStorageArgs(false, storage.staking.payee, ['abc'])
  ...
  it('validates map, 1 arg (failing with no args)', (): void => {
    expect(
      (): any =>
        extractStorageArgs(false, storage.staking.payee, [])
  ...
  it('validates map, 1 arg (failing with no args)', (): void => {
    expect(
      (): any =>
        extractStorageArgs(false, storage.staking.payee, ['abc', 'def'])
    ).toThrow('staking.payee(AccountId) is a map, requiring 1 argument, 2 found');
  });

  it('validates doublemap, 2 args', (): void => {
    expect(
      extractStorageArgs(false, storage.staking.erasStakers, [1, '0x1234'])
  ...
  it('validates doublemap, 2 args (failing with no args)', (): void => {
    expect(
      (): any =>
        extractStorageArgs(false, storage.staking.erasStakers, [])
  ...
  it('validates doublemap, 2 args (failing with 1 arg)', (): void => {
    expect(
      (): any =>
        extractStorageArgs(false, storage.staking.erasStakers, [123])
  ...
  it('validates linked map, no args', (): void => {
    expect(
      extractStorageArgs(false, storage.staking.validators, [])
    ).toEqual([storage.staking.validators]);
  });

  it('validates linked map, single arg', (): void => {
    expect(
      extractStorageArgs(false, storage.staking.validators, [123])
  ...
  it('validates linked map (failing with extra args)', (): void => {
    expect(
      (): any[] =>
        extractStorageArgs(false, storage.staking.validators, [123, 456])
