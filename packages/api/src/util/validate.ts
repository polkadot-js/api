...
function doDoubleMap (creator: StorageEntry, outputType: string, args: any[]): [StorageEntry, [any, any]] {
  const { key1, key2 } = creator.meta.type.asDoubleMap;

  assert(args.length === 2, `${creator.section}.${creator.method}(${key1}, ${key2}): ${outputType} is a doublemap, requiring 2 arguments, ${args.length} found`);

  // pass as tuple
  return [creator, args as [any, any]];
}

function doMap (creator: StorageEntry, outputType: string, args: any[]): [StorageEntry] | [StorageEntry, any] {
  const { key, linked } = creator.meta.type.asMap;
  const sig = `${creator.section}.${creator.method}(${key}): ${outputType}`;
...
export function extractStorageArgs (creator: StorageEntry, args: any[]): any[] {
  ...
  if (creator.meta.type.isDoubleMap) {
    return doDoubleMap(creator, outputType, args);
  } else if (creator.meta.type.isMap) {
    return doMap(creator, outputType, args);
  }

  assert(args.length === 0, `${creator.section}.${creator.method}(): ${outputType} does not take any arguments, ${args.length} found`);
