...
export function extractStorageArgs (creator: StorageEntry, args: any[]): any[] {
  ...
  if (meta.type.isDoubleMap) {
    ...
    assert(args.length === 2, `${section}.${method}([${dmType.key1}, ${dmType.key2}]): ${meta.type} needs an tuple param, ${args.length} found`);

    return [creator, args];
  } else if (meta.type.isMap) {
    const mType = meta.type.asMap;
    const sig = `${section}.${method}(${mType.key}): ${meta.type}`;
    ...
    mType.linked.isTrue
      ...
      : assert(args.length === 1, `${sig} is a map, requiring a single argument, ${args.length} found`);
  } else {
    assert(args.length === 0, `${section}.${method}(): ${meta.type} does not take any arguments, ${args.length} found`);
  }
