...
// creates a submittable class - we are wrapping this whole thing into one monster function
// so we can actually do this on-demand, i.e. we only create right at the point of need
function createSubmittableClass<ApiType> (): Constructor<SubmittableExtrinsic<ApiType>> {
  const _Extrinsic: Constructor<Extrinsic> = ClassOf('Extrinsic');

  return  class Submittable<ApiType> extends _Extrinsic implements SubmittableExtrinsic<ApiType> {
...
let SubmittableClass: Constructor<SubmittableExtrinsic<any>>;
...
export default function createSubmittableExtrinsic<ApiType> (type: ApiTypes, api: ApiInterfaceRx, decorateMethod: ApiBase<ApiType>['decorateMethod']): Creator<ApiType> {
  return (extrinsic: Call | Uint8Array | string): SubmittableExtrinsic<ApiType> => {
    if (!SubmittableClass) {
      SubmittableClass = createSubmittableClass();
    }

    return new SubmittableClass(extrinsic, { api, decorateMethod, type });
  }
}
