...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export type Cons<V, T extends Codec[]> = ((v: V, ...t: T) => void) extends ((...r: infer R) => void)
  ? R
  : never;
...
export type Push<T extends any[], V> = (
  (
    Cons<Codec, Required<T>> extends infer R
      ? {
...
export type ObsInnerCodec<O extends Observable<any>> =
  O extends Observable<infer U>
    ? U extends Codec
      ? U
      : U & Codec
    : never;
...
export interface RxResult<F extends AnyFunction, R = ObsInnerCodec<ReturnType<F>> & Codec> {
  <T = R>(...args: Parameters<F>): Observable<T>;
}

export interface PromiseResult<F extends AnyFunction, E = undefined, R = ObsInnerCodec<ReturnType<F>> & Codec> {
  <T = R> (...args: Parameters<F>): Promise<T>;
  (...args: Push<Parameters<F>, Callback<R, E>>): UnsubscribePromise;
