...
- **Breaking change** The `Data` type has been removed and just replaced with the `U8a` type (In this case it was just an alias, so if any `Data` ws created via `createType`, it should be adjusted)
