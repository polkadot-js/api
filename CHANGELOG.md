...
- **Breaking change** Support latest substrate 2.x v6 metadata with module constants. Some rarely changing and frequently queried parameters have been moved from storage to constants. e.g. `launchPeriod` is directly queriable with `api.constants.launchPeriod`, no longer `api.query.democracy.launchPeriod`.
