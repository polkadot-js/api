...
- **Breaking change** The metadata classes have been moved from `@polkadot/types` into `@polkadot/metadata`. If you explicitly create `Metadata` classes, you now would need to do so with the `import { Metadata } from '@polkadot/metadata';`
- `api.derive.*` now has built-in memomization
- Various fixes and cleanups to `pi.derive.*` including era length progress and vesting calculations
