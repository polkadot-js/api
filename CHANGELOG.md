...
- `Method.injectMethods` is going to be removed. Instead of injecting the methods metadata globally, it is now recommended to pass the function metadata to the `Method` constructor. The function metadata can be retrieved from the runtime metadata using `Method.findMetaByCallIndex`, `Method.findMetaByValue` or `Method.findMetaByName`.
